...
# As more and more scRNA-seq datasets become available, carrying merged_seurat comparisons between them is key.
# There are two main approaches to comparing scRNASeq datasets. The first approach is "label-centric" which is
# focused on trying to identify equivalent cell-types/states across datasets by comparing individual cells or
# groups of cells. The other approach is "cross-dataset normalization" which attempts to computationally remove
# experiment-specific technical/biological effects so that data from multiple experiments can be combined and
# jointly analyzed.
...
# We will running these methods on two human pancreas datasets: [@Muraro2016-yk] and [@Segerstolpe2016-wc]. Since the pancreas
# has been widely studied, these datasets are well annotated.
muraro <- readRDS("data/pancreas/muraro.rds")
segerstolpe <- readRDS("data/pancreas/segerstolpe.rds")

# This data has already been formatted for scmap. Cell type labels must be stored in the `cell_type1` column of
# the `colData` slots, and gene ids that are consistent across both datasets must be stored in the
...
# segerstolpe are match genes in muraro. This is as expected because the segerstolpe dataset was more deeply
# sequenced than the muraro dataset. However, it highlights some of the difficulties in comparing scRNASeq
# datasets.
...
# 
# To simplify our demonstration analyses we will remove the small classes of unassigned cells, and the poor quality cells. We will retain
# the "unclassified endocrine" to see if any of these methods can elucidate what cell-type they belong to.
...
no
library(scmap)
# We recently developed `scmap` [@Kiselev2017-nb] - a method for projecting cells from a scRNA-seq
# experiment onto the cell-types identified in other experiments. Additionally, a cloud version of
# `scmap` can be run for free, withmerged_seurat restrictions, from
...
# You may want to adjust your features using the `setFeatures` function if features are too heavily concentrated in only
# a few cell-types. In this case the dropmerged_seurat-based features look good so we will just them.

# __Exercise__
# Using the rowData of each dataset how many genes were selected as features in both datasets? What does this tell you
# abmerged_seurat these datasets?
...
# scmap computes the distance from each cell to each cell-type in the reference index, then applies an empirically
# derived threshold to determine which cells are assigned to the closest reference cell-type and which are unassigned.
# To account for differences in sequencing depth distance is calculated using the spearman correlation and cosine distance
# and only cells with a consistent assignment with both distances are returned as assigned.
...
# Note that in each case we are projecting to a single dataset but that this could be extended to any number of datasets
# for which we have computed indices.
...
# Again we see cell-types match each other and that all but one of the "PSCs" match the "mesenchymal" cells providing
# strong evidence that these two annotations should be considered synonymous.
...
# This shows the column number of the 5 nearest neighbours in segerstolpe to each of the cells in muraro. We could then
# calculate a pseudotime estimate, branch assignment, or other cell-level data by selecting the appropriate data from the colData of
...
# [Metaneighbour](https://www.biorxiv.org/content/early/2017/06/16/150524) is specifically designed to ask whether cell-type
# labels are consistent across datasets. It comes in two versions. First is a fully supervised method which assumes cell-types
# are known in all datasets and calculates how "good" those cell-type labels are. (The precise meaning of "good" will be described below).
# Alternatively, metaneighbour can estimate how similar all cell-types are to each other both within and across datasets. We will only be
# using the unsupervised version as it has much more general applicability and is easier to interpret the results of.

# Metaneighbour compares cell-types across datasets by building a cell-cell spearman correlation network. The method then tries
# to predict the label of each cell through weighted "votes" of its nearest-neighbours. Then scores the overall similarity between
# two clusters as the AUROC for assigning cells of typeA to typeB based on these weighted votes. AUROC of 1 would indicate all the
# cells of typeA were assigned to typeB before any other cells were, and an AUROC of 0.5 is what you would get if cells were being
# randomly assigned.
...
## Debugged -> Check out https://github.com/satijalab/seurat/issues/1134
...
muraro_seurat <- NormalizeData(object=muraroseurat)
muraro_seurat <- ScaleData(object=muraroseurat)

## Debugged -> Check out https://github.com/satijalab/seurat/issues/1113
muraro_seurat <- FindVariableFeatures(object=muraroseurat, do.plot=TRUE)

seger_seurat <- NormalizeData(object=segerseurat)
seger_seurat <- ScaleData(object=segerseurat)
seger_seurat <- FindVariableFeatures(object=segerseurat, do.plot=TRUE)
...
# Debugged -> Check out https://github.com/satijalab/seurat/issues/544
...
seger_hvg <- seger_hvg[order(seger_hvg, decreasing=TRUE)]
muraro_hvg <- seger_seurat@assays$RNA@var.features
muraro_hvg <- muraro_hvg[order(muraro_hvg, decreasing=TRUE)]
...
##If shows invalid graphics state, run dev.off() and execute the command again
...
### BUG in CaclVarExpRatio. Cannot find this function
